...
public func resizePixelBuffer(_ pixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
   ...
   let widthBuf  = CVPixelBufferGetWidth(pixelBuffer)
       let heightBuf = CVPixelBufferGetHeight(pixelBuffer)

       let height0 = ceil(Double(widthBuf) / 1.58)
       let y0 = (heightBuf - Int(height0)) / 2

       return resizePixelBuffer(pixelBuffer,
                                cropX: 0,
                                cropY: Int(y0),
                                cropWidth: Int(widthBuf),
                                cropHeight: Int(height0),
                                scaleWidth: Int(widthBuf),
